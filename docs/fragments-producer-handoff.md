...
## Current Parity Status (sample.ifc)
...
249 elements in sample.ifc use these types and produce no geometry in the native.
...
- model A uses IFCINDEXEDCOLOURMAP (1,426 entities) for per-face vertex colors — may be the source of the missing 11
...
4. Dead test referencing removed test.ifc → sample.ifc
...
# Run on model A
target/release/ifc2lbd-neo web/wasm-prototype/public/sample.ifc \
...
python3 scripts/validate_fragments_parity.py web/wasm-prototype/public/sample.ifc

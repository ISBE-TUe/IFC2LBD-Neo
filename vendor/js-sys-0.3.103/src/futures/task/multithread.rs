...
impl AtomicWaker {
    ...
    fn wake_by_ref(&self) {
        ...
        unsafe {
            // Use `core::arch::wasm` (target_family = "wasm") which works
            // on both wasm32 and wasm64.  Falls back to `wasm32`/`wasm64`
            // for older toolchains that don't have the unified module.
            #[cfg(target_arch = "wasm64")]
            core::arch::wasm64::memory_atomic_notify(
                self.state.as_ptr(),

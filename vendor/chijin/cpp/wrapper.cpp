    ...
    double x2, double y2, double z2)
{
    ...
    // BRepPrimAPI_MakeBox throws Standard_DomainError for zero/negative dimensions.
    // Inflate degenerate axes to 1mm so flat/planar elements don't crash.
    const double MIN_DIM = 0.001;
    double dx = std::max(maxx - minx, MIN_DIM);
    double dy = std::max(maxy - miny, MIN_DIM);
    double dz = std::max(maxz - minz, MIN_DIM);
...
rust::Vec<double> shape_bounding_box(const TopoDS_Shape& shape) {
...
// Build a closed solid from a triangle mesh (for IfcTriangulatedFaceSet / IfcPolygonalFaceSet).
// vertices: flat [x0,y0,z0, x1,y1,z1, ...] list of vertex positions
// indices:  flat [a0,b0,c0, a1,b1,c1, ...] 0-based triangle vertex indices
// Returns null on failure.
std::unique_ptr<TopoDS_Shape> solid_from_triangle_mesh(
    rust::Slice<const double> vertices,
    rust::Slice<const int32_t> indices)
{
    if (vertices.size() < 9 || vertices.size() % 3 != 0) return nullptr;
    if (indices.size() < 3 || indices.size() % 3 != 0) return nullptr;
    size_t nv = vertices.size() / 3;
    size_t nt = indices.size() / 3;
    // Validate indices.
    for (size_t i = 0; i < indices.size(); ++i) {
        if (indices[i] < 0 || (size_t)indices[i] >= nv) return nullptr;
    }
    try {
        // Build shared vertex/edge topology directly from the mesh index data.
        // This is O(n log n) and avoids BRepBuilderAPI_Sewing's O(n²) geometric
        // edge-coincidence detection, which hangs for meshes > ~100 triangles when
        // all vertices are positionally unique.

        // Step 1: One OCC vertex per mesh vertex.
        std::vector<TopoDS_Vertex> occ_verts(nv);
        for (size_t v = 0; v < nv; ++v) {
            occ_verts[v] = BRepBuilderAPI_MakeVertex(
                gp_Pnt(vertices[v*3], vertices[v*3+1], vertices[v*3+2]));
        }

        // Step 2: One OCC edge per unique undirected index pair (min,max).
        // Canonical orientation: occ_verts[min] → occ_verts[max].
        std::map<std::pair<int,int>, TopoDS_Edge> edge_map;
        for (size_t t = 0; t < nt; ++t) {
            for (int e = 0; e < 3; ++e) {
                int a = indices[t*3+e], b = indices[t*3+(e+1)%3];
                auto key = std::make_pair(std::min(a,b), std::max(a,b));
                if (edge_map.count(key) == 0) {
                    int lo = std::min(a,b), hi = std::max(a,b);
                    edge_map[key] = BRepBuilderAPI_MakeEdge(occ_verts[lo], occ_verts[hi]);
                }
            }
        }

        // Step 3: One planar BRep face per triangle using shared edges.
        // Adjacent triangles share the same TopoDS_Edge object → no sewing required.
        BRep_Builder builder;
        TopoDS_Shell shell;
        builder.MakeShell(shell);

        for (size_t t = 0; t < nt; ++t) {
            int idx[3] = { indices[t*3], indices[t*3+1], indices[t*3+2] };
            gp_Pnt pts[3];
            for (int k = 0; k < 3; ++k) {
                int vi = idx[k];
                pts[k] = gp_Pnt(vertices[vi*3], vertices[vi*3+1], vertices[vi*3+2]);
            }
            gp_Vec n = gp_Vec(pts[0], pts[1]).Crossed(gp_Vec(pts[0], pts[2]));
            ...
            for (int e = 0; e < 3; ++e) {
                ...
                auto key = std::make_pair(std::min(a,b), std::max(a,b));
                TopoDS_Edge edge = edge_map.at(key);
                // If the triangle uses this edge in the non-canonical direction, reverse it.
                if (a > b) edge.Reverse();
                mw.Add(edge);
            }
        ...
        return std::make_unique<TopoDS_Shape>(ms.Solid());
    } catch (const Standard_Failure&) {
        return nullptr;
    }
}

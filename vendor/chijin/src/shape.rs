...
impl Shape {
	...
	pub fn write_step_with_colors(&self, writer: &mut impl Write) -> Result<(), Error> {
		...
		let r: Vec<f32> = ids
			.iter()
			.map(|&id| self.colormap[&TShapeId(id)].r)
			.collect();
		let g: Vec<f32> = ids
			.iter()
			.map(|&id| self.colormap[&TShapeId(id)].g)
			.collect();
		let b: Vec<f32> = ids
			.iter()
			.map(|&id| self.colormap[&TShapeId(id)].b)
			.collect();
	...
	/// Build a closed solid from a triangle mesh.
	///
	/// `vertices`: flat `[x0, y0, z0, x1, y1, z1, ...]`
	/// `indices`: flat 0-based triangle indices `[a0, b0, c0, a1, b1, c1, ...]`
	///
	/// Uses OCC `BRepBuilderAPI_Sewing` + `BRepBuilderAPI_MakeSolid`.
	/// Returns `None` if the mesh is degenerate, not closed, or OCC fails.
	pub fn from_triangle_mesh(vertices: &[f64], indices: &[i32]) -> Option<Shape> {
		let inner = ffi::solid_from_triangle_mesh(vertices, indices);
		if inner.is_null() {
			None
		} else {
			Some(Shape {
...
impl Shape {
	...
	/// Compute the axis-aligned bounding box of this shape using OCC `BRepBndLib`.
	/// Returns `Some([xmin,ymin,zmin,xmax,ymax,zmax])` or `None` if the shape has no geometry.
	pub fn bounding_box(&self) -> Option<[f64; 6]> {
		let v = ffi::shape_bounding_box(&self.inner);
		if v.len() == 6 {

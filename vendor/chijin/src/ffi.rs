...
mod ffi_bridge {
	...
	unsafe extern "C++" {
		...
		/// Returns [xmin,ymin,zmin,xmax,ymax,zmax] or empty Vec if shape has no geometry.
		fn shape_bounding_box(shape: &TopoDS_Shape) -> Vec<f64>;
		...
		fn solid_from_triangle_mesh(vertices: &[f64], indices: &[i32]) -> UniquePtr<TopoDS_Shape>;

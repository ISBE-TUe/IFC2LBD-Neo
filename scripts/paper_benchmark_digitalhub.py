...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Repeated model A benchmark for the EG-ICE paper"
    )
    parser.add_argument("--root", type=Path, default=root)
    parser.add_argument("--ifc", type=Path, default=root / "model-A.ifc")
    parser.add_argument(
        "--out-dir",
        type=Path,
...
def render_markdown(report: dict[str, object]) -> str:
    ...
    lines.extend(
        [
            ...
            "",
            "- Table: model A repeated baseline and Rust mode comparison with mean ± sd for wall time, peak RSS, and output size.",
            "- Figure: Baseline Comparison model A Arch (LBD+IfcOWL), reported as `mu ± sigma`, optionally annotated with `n=20` in the caption.",
...
def plot_baseline(path: Path, aggregates: dict[str, AggregateResult]) -> None:
    ...
    fig.suptitle("Baseline Comparison model A Arch (LBD+IfcOWL)")
...
def plot_modes(path: Path, aggregates: dict[str, AggregateResult]) -> None:
    ...
    fig.suptitle("Mode Comparison model A Arch (IFC2LBD-Neo)")
    fig.tight_layout(rect=[0, 0, 1, 0.95])
    fig.savefig(path, dpi=180, bbox_inches="tight")
    plt.close(fig)

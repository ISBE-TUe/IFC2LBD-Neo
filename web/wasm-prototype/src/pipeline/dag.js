...
function render() {
	...
	const { activeModules, modules, stageStatuses, running, selectedPluginId } =
		state;
	...
	for (let si = 0; si < stageIds.length - 1; si++) {
		...
		for (const fromId of fromIds) {
			...
			for (const toId of toIds) {
				...
				const fromMod = modules.find((m) => m.id === fromId) || {
					stage: "Preprocess",
				};
				...
				drawConnection(
					fromPos,
					toPos,
					running && !stageStatuses[fromId]?.status,
				);
			}
		}
	}
...
function drawNode(id, pos, status, isSelected, isRunning) {
	...
	rect.setAttribute(
		"stroke",
		isSelected ? "#4A9EDA" : STAGE_COLORS[status] || STAGE_COLORS.idle,
	);
	...
	name.setAttribute(
		"y",
		pos.y + (statusData?.durationMs ? pos.h / 2 - 6 : pos.h / 2 + 4),
	);
	...
	if (statusData?.durationMs) {
		const timing = document.createElementNS(
			"http://www.w3.org/2000/svg",
			"text",
		);
	...
	// Error indicator
	if (status === "failed") {
		const errText = document.createElementNS(
			"http://www.w3.org/2000/svg",
			"text",
		);
		...
		const okText = document.createElementNS(
			"http://www.w3.org/2000/svg",
			"text",
		);
